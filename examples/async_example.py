...
# 导入ORM模块
from xtsqlorm import AsyncOrmOperations, AsyncSqlConnection, BaseModel
...
class AsyncExample:
    ...
    async def test_crud_operations(self) -> None:
        ...
        try:
            ...
            # 1. 插入单条记录
            user_data = {"username": "async_user1", "email": "async_user1@example.com"}
            ...
            batch_update_result = await self.orm_ops.update_many(
                batch_update_data, User.id.in_([2, 3])
            )
        ...
        try:
            ...
            async with self.connection.transaction():
                ...
                await self.orm_ops.insert(
                    {"username": "transaction_user", "email": "transaction@example.com"}
                )
