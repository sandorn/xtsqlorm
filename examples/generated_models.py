...
class Users(Base):
    __tablename__ = 'users'
    __table_args__ = (Index('email', 'email', unique=True), Index('username', 'username', unique=True))
    ...
    is_active: Mapped[int | None] = mapped_column(TINYINT(1))
    is_admin: Mapped[int | None] = mapped_column(TINYINT(1))
