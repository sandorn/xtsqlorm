...
from xtsqlorm.core.async_operations import AsyncOrmOperations
from xtsqlorm.core.connection import SqlConnection
from xtsqlorm.core.operations import OrmOperations
from xtsqlorm.models.base import BaseModel
from xtwraps.log import log_wraps, mylog as log
...
def create_orm_operations(
    ...
    **conn_kwargs: Any,
    ...
    # 验证数据模型类型
    is_valid_type = (
        isinstance(source_model, type) and issubclass(source_model, BaseModel)
    ) or isinstance(source_model, str)

    if not is_valid_type:
        error_msg = f"无效的数据模型类型: {type(source_model).__name__}"
        log.warning(f"{create_orm_operations.__name__} | {error_msg}")
    ...
    if isinstance(source_model, str) and copy_name:
        ...
        if not inspector.has_table(table_name):
            ...
            log.ok(f"{create_orm_operations.__name__} | 成功创建表: {table_name}")

    # 创建ORM操作实例
    orm_ops = OrmOperations(model_class, db_conn)
    # 修复日志记录，使用正确的属性名  类名：__name__ ，表名： __tablename__
    source_name = (
        source_model if isinstance(source_model, str) else source_model.__tablename__
    )
    log.ok(
        f"{create_orm_operations.__name__} | 创建ORM操作: {model_class.__name__} | 表: {model_class.__tablename__} | 源: {source_name}"
    )
...
def reflect_table(
    ...
    **conn_kwargs: Any,
    ...
    # 只反射指定的表以提高性能
    log.start(f"{reflect_table.__name__} | 正在反射数据库表: {source_table_name}")
    ...
    log.ok(
        f"{reflect_table.__name__} | 成功反射表: {source_table_name}，创建模型类: {model_name}"
    )
...
def copy_table_model(
    ...
    **conn_kwargs: Any,
    ...
    if not inspector.has_table(new_table_name):
        ...
        log.ok(f"✅ {copy_table_model.__name__} | 成功创建表: {new_table_name}")
    else:
        log.warning(
            f"ℹ️ {copy_table_model.__name__} | 表已存在，跳过创建: {new_table_name}"
        )
    ...
    log.ok(
        f"{copy_table_model.__name__} | 成功复制表: {source_table_name} 为 {new_table_name}，创建模型类: {model_name}"
    )
...
def _validate_tablename(tablename: str) -> None:
    ...
    if (
        not tablename
        or not isinstance(tablename, str)
        or not re.match(r"^[a-zA-Z0-9_]+$", tablename)
    ):
...
def _build_command_args(
    url: str, tablename: str, output_file: str, **kwargs: Any
) -> list[str]:
...
def _execute_command(
    cmd_args: list[str], echo: bool = False
        ...
        for arg in cmd_args:
            if "://" in arg and ":" in arg.split("://")[1]:
                ...
                if len(credentials_and_rest) > 1:
                    ...
                    safe_cmd.append(
                        f"{parts[0]}://{credentials}@{credentials_and_rest[1]}"
                    )
                else:
                    safe_cmd.append(arg)
            else:
                safe_cmd.append(arg)
        log.info(f"{_execute_command.__name__} | 执行命令: {' '.join(safe_cmd)}")
...
def db_to_model(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if not url:
            url = connect_str(db_key)
            log.start(f"{db_to_model.__name__} | 从配置获取连接信息，配置键: {db_key}")
        else:
            log.start(f"{db_to_model.__name__} | 使用自定义连接URL")
        ...
        if result.stdout and echo:
            log.info(f"{db_to_model.__name__} | 命令输出: {result.stdout.strip()}")

        if result.stderr:
            log.warning(
                f"{db_to_model.__name__} | 命令错误输出: {result.stderr.strip()}"
            )

        if result.returncode == 0:
            log.ok(f"{db_to_model.__name__} | 成功生成模型文件: {output_file}")
        else:
            log.fail(
                f"{db_to_model.__name__} | 生成模型文件失败，返回码: {result.returncode}"
            )
        ...
        log.error(f"{db_to_model.__name__} | {error_msg}")
...
async def create_async_orm_operations(
    ...
    **conn_kwargs: Any,
    ...
    if not is_valid_type:
        error_msg = f"无效的数据模型类型: {type(source_model).__name__}"
        log.warning(f"{create_async_orm_operations.__name__} | {error_msg}")
        raise ValueError(error_msg)
    ...
    if isinstance(source_model, str):
        ...
        if not inspector.has_table(table_name):
            ...
            log.ok(f"{create_async_orm_operations.__name__} | 成功创建表: {table_name}")
        else:
            log.warning(
                f"{create_async_orm_operations.__name__} | 表已存在，跳过创建: {table_name}"
            )
    ...
    log.ok(
        f"{create_async_orm_operations.__name__} | 创建异步ORM操作: {model_class.__name__} | 表: {model_class.__tablename__} | 源: {source_name}"
    )
...
async def reflect_table_async(
    ...
    log.start(
        f"{reflect_table_async.__name__} | 正在异步反射数据库表: {source_table_name}"
    )

    # 使用同步引擎进行反射（简化实现）
    await db_conn.engine.run_sync(
        lambda engine: BaseModel.metadata.reflect(engine, only=[source_table_name])
    )
    ...
    log.ok(
        f"{reflect_table_async.__name__} | 成功异步反射表: {source_table_name}，创建模型类: {model_name}"
    )
    return table_model
...
if __name__ == "__main__":
    ...
    from xtwraps.log import mylog as loger

    # 创建命令行参数解析器
    parser = argparse.ArgumentParser(description="将数据库表转换为SQLAlchemy模型类文件")
    parser.add_argument(
        "tablename", nargs="?", default="users2", help="要转换的数据库表名"
    )
    parser.add_argument(
        "--db-key", default="default", help='数据库配置键，默认为"default"'
    )
    parser.add_argument(
        "--output", default=None, help="输出文件路径，默认为tablename_db.py"
    )
    ...
    if not args.tablename:
        ...
        loger.info(
            f"  python {__file__} your_table_name --output=custom_model.py --echo"
        )
        loger.info("\n注意：")
