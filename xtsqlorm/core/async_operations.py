...
class AsyncOrmOperations[T]:
    ...
    async def get_by_id(self, id_value: int, session: AsyncSession | None = None) -> T | None:
        ...
        Returns:
            查询到的模型对象,不存在则返回None
        """
        session = session or await self.db.new_session()
        try:
    ...
    async def insert_many(self, data_list: list[dict[str, Any]], tablename: str | None = None) -> list[int]:
        ...
        coro = [self.query(sql, *params) if isinstance(sql, tuple) else self.query(sql, params) for sql, params in sql_params_list]

        # 使用事件循环运行所有协程
        return self.db.loop_run(coro)
    ...
    async def update_many(
        ...
        return self.db.loop_run(coro_list)
    ...
    @log_wraps
    async def delete(self, where_dict: dict[str, Any], session: AsyncSession | None = None) -> int | None:
        """
        异步删除符合条件的记录
